	...
	$.extend(Plugin.prototype, {
		...
		init: function () {
			...
			if (this.settings.autoComplete === true) {
				...
				autoPlaces.setComponentRestrictions({'country': ['ae']});
		...
		_start: function () {
			...
			if (window.matchMedia("(max-width: 767px)").matches){ // only for mobile device
...
/* 					$.getJSON('http://ip-api.com/json').done(function(data){
			...
			if (this.settings.querystringParams === true) {
				...
				if (this.getQueryString(this.settings.maxDistanceID)){
					...
					if ($distanceInput.val() !== '') {
						distance = $distanceInput.val() || this.settings.distanceAlert;
						distance = parseFloat(distance);
					}
				}
				else{
		...
		getAddressByMarker: function(markerID) {
			...
			if(locationset[markerID].postcode){ formattedAddress += locationset[markerID].postcode + ' '; }
			...
			if (destination) {
				...
				directionsService.route(request, function(response, status) {
					if (status === google.maps.DirectionsStatus.OK) {
						$('.' + self.settings.locationList + ' ul').hide();
						directionsDisplay.setDirections(response);
						$('.' + self.settings.locationList).prepend('<div class="bh-sl-close-directions-container"><div class="' + self.settings.closeIcon + '"></div></div>');
						$(document).off('click', '.' + self.settings.locationList + ' li .loc-directions a');
					} else {
						require(['Magento_Ui/js/modal/alert'],function(alert){
							alert({
								title: 'Notify',
								content: 'Sorry, we could not calculate directions from "'+origin+'" to "'+destination+'"',
								actions: {
									always: function(){}
								}
							});
