...
class Deleteimileorder extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        $expressNo       = $this->getRequest()->getParam('express_no');
		...
		if ($order_id != '') {
			...
			if (isset($rnrOrderResponse['status']) && $rnrOrderResponse['status'] == 200) {
				$subject = 'Sales order deleted in iMile for order #'.$order->getIncrementId();
				...
				$order->addStatusToHistory($order->getStatus(), 'Order deleted successfully with reference ExpressNo:'.$expressNo);
				$order->setImileOrderResponse('');
				$order->setImileOrderTrackingResponse('');
				$order->save();
				$this->_messageManager->addSuccess(__('imile Delivery Order deleted successfully.'));
				...
				$this->_messageManager->addError(__('iMile Delivery Order not deleted.'));
				$this->_redirect('sales/order/view', array('order_id' => $order_id));
			}
		}else{
