...
class Getengine extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $modelengine_url = "https://api.wheel-size.com/v2/search/by_model/?user_key=" . $wheelApiKey . "&make=" . $make . "&model=" . $model . "&year=" . $year. "&region=medm";
        ...
        foreach ($modelengine->data as $key => $enginevalue) {
            ...
            foreach ($enginevalue->wheels as $tyrekey => $tyrevalue) {
				...
				if ($tyrevalue->rear->tire_width) {
                    ...
                    if (isset($enginesTyre[$slug])) {
                        $enginesTyre[$tyrevalue->front->tire . ' ' . $tyrevalue->rear->tire].= '<li class="'.$slug.'" ><a href="javascript:void(0)" onclick="showproduct(\'' . $frontwidth . '\',\'' . $frontheight . '\',\'' . $frontrim . '\')" >' . $tyrevalue->front->tire . ' ' . $tyrevalue->rear->tire . '</a></li>';
                    } else {
