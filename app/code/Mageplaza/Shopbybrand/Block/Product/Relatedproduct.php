...
class Relatedproduct extends ListProduct
{
    ...
    */
	...
	public function isRearTyreset()
    {
         $rear_width_dropdown    = $this->getRequest()->getParam('rw');
         $rear_profile_dropdown  = $this->getRequest()->getParam('rp'); 
         $rear_diameter_dropdown = $this->getRequest()->getParam('rd'); 
         $isRearset=0;
         if(isset($rear_width_dropdown) && isset($rear_profile_dropdown) && isset($rear_diameter_dropdown) ){
             $isRearset=1;
         }
	...
	public function getRearcollection()
	{
		...
		$rear_tyre_search_collection = $this->productCollectionFactory->create()
							...
							->addFieldToFilter('width',$rear_width_dropdown)
							->addFieldToFilter('profile',$rear_profile_dropdown)
							->addFieldToFilter('diameter',$rear_diameter_dropdown);

        return $rear_tyre_search_collection;
	}
	
	public function getRearCount()
    {
        $rear_tyre_search_count=0;

        if(count($this->getRearcollection()) > 0 ){
            $rear_tyre_search_count=1;    
        } 

        return $rear_tyre_search_count;
    }
	
	public function isBundleProduct()
    {  
        $frontSearchvalue = array();
        $frontSearchvalue[] = $this->getRequest()->getParam('fw');               
        $frontSearchvalue[] = $this->getRequest()->getParam('fp');
        $frontSearchvalue[] = $this->getRequest()->getParam('fd'); 
 
        $rearSearchvalue = array();
        $rearSearchvalue[] = $this->getRequest()->getParam('rw');
        $rearSearchvalue[] = $this->getRequest()->getParam('rp'); 
        $rearSearchvalue[] = $this->getRequest()->getParam('rd'); 

        $isBundleProduct = 1;
        if($frontSearchvalue === $rearSearchvalue ){
             $isBundleProduct = 0;
        }
