...
class Router implements \Magento\Framework\App\RouterInterface
{
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if ($routeSize == 1) {
			...
			if($routePath[1] && isset($routePath[2])){
				$request->setControllerName('index')->setActionName('patternview')
                        ->setParam('pattern_key', $routePath[2])->setPathInfo(
                            '/mpbrand/index/patternview'
                        );
			}else{
				switch ($routePath[1]) {
